from postpitch import app, db, bcrypt
import os
import secrets
from PIL import Image
from flask import render_template, url_for, flash, redirect, request, abort
from postpitch.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm
from postpitch.models import User, Pitch
...
def new_pitch():
  form = PostForm()
  if form.validate_on_submit():
    pitch = Pitch(pitch=form.pitch.data, comment=form.comment.data, user=current_user)
...
def pitch(pitch_id):
  ...
  return render_template ('pitch.html', title=pitch.pitch, pitch=pitch)
...
def user_pitch(username):
  ...
  pitches = Pitch.query.filter_by(user=user).order_by(Pitch.date_posted.desc()).paginate(page=page, per_page=5)
